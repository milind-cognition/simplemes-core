...
<template>
  <Dialog v-model:visible="dialogVisible" :breakpoints="{'960px': '95vw', '640px': '100vw'}" :style="{width: '90vw'}"
    ...
    <div class="fluid formgrid grid align-items-center">
      ...
      <Tabs v-if="fields.tabs.length>0" class="col-12" :value="fields.tabs[0]?.tab">
        <TabList>
          <Tab v-for="tab in fields.tabs" :key="tab.tab" :value="tab.tab">{{ $t(tab.tabLabel) }}</Tab>
        </TabList>
        <TabPanels>
          <TabPanel v-for="tab in fields.tabs" :key="tab.tab" :value="tab.tab">
            <div class="fluid formgrid grid align-items-center">
